...
class PHPMailer {
  ...
  final public const STOP_MESSAGE = 0; // message only, continue processing
  final public const STOP_CONTINUE = 1; // message?, likely ok to continue processing
  final public const STOP_CRITICAL = 2; // message, plus full stop, critical error reached
   ...
   * @access private
   */
  private function AddAnAddress($kind, $address, $name = ''): bool {
...
/**
  ...
  public static function ValidateAddress($address) {
    if (function_exists('filter_var')) { //Introduced in PHP 5.2
      ...
      return preg_match('/^(?:[\w\!\#\$\%\&\'\*\+\-\/\=\?\^\`\{\|\}\~]+\.)*[\w\!\#\$\%\&\'\*\+\-\/\=\?\^\`\{\|\}\~]+@(?:(?:(?:\w(?:[a-zA-Z0-9_\-](?!\.)){0,61}[a-zA-Z0-9_-]?\.)+\w(?:[a-zA-Z0-9_\-](?!$)){0,61}\w?)|(?:\[(?:(?:[01]?\d{1,2}|2[0-4]\d|25[0-5])\.){3}(?:[01]?\d{1,2}|2[0-4]\d|25[0-5])\]))$/', $address);
    }
  }
   ...
   * @access protected
   */
  protected function MailSend($header, $body): bool {
    $toArr = [];
    ...
    } elseif ($this->SingleTo && count($toArr) > 1) {
        foreach ($toArr as $val) {
          $rt = @mail($val, $this->EncodeHeader($this->SecureHeader($this->Subject)), $body, $header, $params);
        }
    } else {
  ...
  protected function SmtpSend($header, $body): bool {
    ...
    if ($bad_rcpt !== [] ) { //Create error message for any bad addresses
  ...
  public function SmtpConnect(): bool {
    ...
    try {
      while($index < count($hosts) && !$connection) {
        $hostinfo = [];
        if (preg_match('/^(.+):(\d+)$/', $hosts[$index], $hostinfo)) {
          $host = $hostinfo[1];
          $port = $hostinfo[2];
        } else {
          $host = $hosts[$index];
  ...
  */
  ...
  public function WrapText($message, $length, $qp_mode = false) {
    ...
    foreach ($line as $singleLine) {
        $line_part = explode(' ', $singleLine);
        $buf = '';
        foreach ($line_part as $e => $singleLine_part) {
            $word = $singleLine_part;
  ...
  public function CreateHeader() {
    ...
    // Add custom headers
    foreach ($this->CustomHeader as $singleCustomHeader) {
        $result .= $this->HeaderLine(trim((string) $singleCustomHeader[0]), $this->EncodeHeader(trim((string) $singleCustomHeader[1])));
    }
  ...
  public function CreateBody() {
    ...
    if ($this->IsError()) {
      ...
      try {
        ...
        if (@openssl_pkcs7_sign($file, $signed, "file://".$this->sign_cert_file, ["file://".$this->sign_key_file, $this->sign_key_pass], NULL)) {
          @unlink($file);
          @unlink($signed);
   ...
   * @access private
   */
  private function GetBoundary($boundary, $charSet, $contentType, $encoding): string {
    $result = '';
    if($charSet == '') {
   ...
   * @access private
   */
  private function EndBoundary($boundary): string {
   ...
   */
  public function AddAttachment($path, $name = '', $encoding = 'base64', $type = 'application/octet-stream'): bool {
    try {
      if ( !@is_file($path) ) {
  ...
  */
  ...
  private function EncodeFile($path, $encoding = 'base64') {
    try {
      ...
      if (function_exists('get_magic_quotes')) {
        function get_magic_quotes(): bool {
          return false;
        }
      }
      if (PHP_VERSION < 6) {
  ...
  public function EncodeString ($str, $encoding = 'base64') {
    ...
    switch(strtolower($encoding)) {
      ...
      case '8bit':
        ...
        if (!str_ends_with($encoded, $this->LE))
          $encoded .= $this->LE;
        break;
      case 'binary':
        $encoded = $str;
   ...
   * @param string $str multi-byte text to wrap encode
   */
  public function Base64EncodeWrapMB($str): string {
  ...
  */
   ...
   * @param string $type File extension (MIME) type.
   */
  public function AddEmbeddedImage($path, $cid, $name = '', $encoding = 'base64', $type = 'application/octet-stream'): bool {
  ...
  public function MsgHTML($message, $basedir = '') {
    preg_match_all("/(src|background)=\"(.*)\"/Ui", (string) $message, $images);
    if(isset($images[2])) {
      foreach($images[2] as $i => $url) {
        ...
        if (!preg_match('#^[A-z]+://#',(string) $url)) {
          ...
          if ( strlen((string) $basedir) > 1 && !str_ends_with((string) $basedir, '/')) { $basedir .= '/'; }
          if ( strlen($directory) > 1 && !str_ends_with($directory, '/')) { $directory .= '/'; }
          if ( $this->AddEmbeddedImage($basedir.$directory.$filename, md5($filename), $filename, 'base64',$mimeType) ) {
            $message = preg_replace("/".$images[1][$i]."=\"".preg_quote((string) $url, '/')."\"/Ui", $images[1][$i]."=\"".$cid."\"", (string) $message);
          }
        }
      }
    }
    $this->IsHTML(true);
    $this->Body = $message;
    $textMsg = trim(strip_tags(preg_replace('/<(head|title|style|script)[^>]*>.*?<\/\\1>/s','',(string) $message)));
...
';
  ...
  public static function _mime_types($ext = '') {
    ...
    return $mimes[strtolower((string) $ext)] ?? 'application/octet-stream';
  }
  ...
  */
  public function set($name, mixed $value = ''): bool {
    try {
      if (isset($this->$name) ) {
        $this->$name = $value;
      } else {
