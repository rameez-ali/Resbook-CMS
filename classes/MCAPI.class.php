...
class MCAPI {
     ...
     *
     * @param string $api_key Your MailChimp apikey
    ...
    function useSecure($val){
        $this->secure = $val===true;
    }
    
    public function preprint(&$string){
        echo "<pre>";
        print_r($string);
        echo "</pre>";
    }
    ...
    * @param array $type_opts optional - 
     ...
     */
    function campaignUpdate($cid, $name, mixed $value) {
        $params = [];
        $params["cid"] = $cid;
        $params["name"] = $name;
        $params["value"] = $value;
    ...
    */
     ...
     */
    function templates($types=[], $category=NULL, $inactives=[]) {
        $params = [];
        $params["types"] = $types;
     ...
     */
    function generateText($type, mixed $content) {
        $params = [];
        $params["type"] = $type;
        $params["content"] = $content;
    ...
    function callServer($method, $params) {
        ...
        foreach($headers as $h){
            if (str_starts_with($h, "X-MailChimp-API-Error-Code")){
                $errored = true;
