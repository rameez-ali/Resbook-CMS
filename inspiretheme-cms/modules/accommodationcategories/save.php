...
function saveItem (){
  ...
  $arrItemData['iframe_code_accomm'] = requestVar('iframe_code_accomm');
  ...
  if (!empty($metaDataId) && !empty($id)) {
    ...
    /** Update Product data */
    DB::updateRow($arrItemData, 'accommodation_category', "WHERE id = '{$id}' LIMIT 1");
    ...
    $metaDataId = DB::insertRow($arrMetaData, 'page_meta_data');	
      ...
      $id = DB::insertRow( $arrItemData, 'accommodation_category' );
  ...
  $message = ucfirst(strtolower($modMsgLabel))." has been saved";
