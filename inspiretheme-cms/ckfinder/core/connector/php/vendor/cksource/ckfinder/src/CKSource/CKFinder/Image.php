...
class Image
{
    protected static $supportedExtensions = ['jpg', 'jpeg', 'gif', 'png', 'svg', 'ico'];
    ...
    public function __construct($imageData, $bmpSupport = false)
    {
        ...
        if (strpos($imageData, '<svg') !== false) {
            ...
            $this->svgContent = $imageData;
            return; // Exit the constructor early since it's an SVG
        }
        ...
        } elseif (strpos($imageData, '<svg') !== false) {
            $this->mime = 'image/svg+xml';
        } elseif (substr($imageData, 0, 4) === "\x00\x00\x01\x00") {
        ...
        if ($this->mime === 'image/svg+xml') {
            $this->svgContent = $imageData;
            return; // Return early since SVG doesn't need GD processing
        }
        ...
        if ($this->mime === 'image/x-icon') {
            $this->iconContent = $imageData;
        } elseif ('image/bmp' === $this->mime || 'image/x-ms-bmp' === $this->mime) {
