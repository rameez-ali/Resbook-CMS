...
function initMain()
{
  ...
  global $do, $id, $message, $itemSelect, $itemRank, $moduleMainHeading, $moduleSubHeading, $modMsgLabel, $listType, $modName;
  $moduleContent      = (empty($moduleContent)) ? '' : $moduleContent;
  $template           = (empty($template)) ? '' : $template;
  $moduleMainHeading = 'Highlights';
  $modMsgLabel       = 'Highlight';
  $modName           = 'Highlight';
  ...
  switch ($action) {
    case 'publish':
      $message = ModuleAction::publish($itemSelect,'highlight');
      break;

    case 'hide':
      $message = ModuleAction::hide($itemSelect,'highlight');
      break;

    case 'delete':
      $message = ModuleAction::delete($itemSelect,'highlight');
      break;

    case 'saverank':  
      $message = ModuleAction::saveRank($itemRank,'highlight');
      break;

    case 'trash': 
      $listType = FLAG_DELETED;
      $message = ($initRestore) ? ModuleAction::restore($itemSelect,'highlight') : '';      
      break;
